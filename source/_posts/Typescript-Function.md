...
我们知道在Js中函数是一个对象, 可以像下面这样往函数中添加属性. (当然我们很少这么做,只是使用Function原有的属性`name`,`length`等)
...
在Js中,还可以使用`new`操作符调用函数.这时候我们可以在调用签名前添加一个`new`,便可以声明构造函数的目的.对于调用时`new`操作符是可选的函数(对象),比如 `Date`, 需要同时声明调用签名,与构造签名
...
函数很简单,我们希望函数能够根据我们输入的类型自动得出返回值的类型,当我输入`string[]`类型时,返回值就是`string`类型的,其他类型同理.但实际上由于是任意数组,无法事先知道参数的类型,目前为止只能使用`any[]`类型,这时候返回类型自然也就是`any`类型.不满足我们的要求! 这就轮到**泛型函数**登场了
...
可以看到,我们在函数名后方声明了类型`Type`,并在参数中使用`Type[]`对arr进行注解,函数返回类型也是`Type`类型.这样我们就得到了一个通用的类型函数(**泛型函数**).函数的返回类型会根据参数类型自动推断出来.

{% note  info %}

注意: 不需要主动去指定`Type`.Ts会自动推断出正确的类型
...
另外对于返回泛型`Type`的函数, 返回值满足约束条件并不一定能满足类型`Type`,`Type`是由入参决定的,`Type`是“约束”的超集.
...
对于泛型函数,若指定多个入参均为泛型`Type`,这些入参的类型应完全一样.对于不一样的类型,可以在调用函数时指定通用泛型
...
2. 尽可能的少用泛型
   ...
   对比`filter1`与`filter2`,不难发现两种写法,在不指定类型其实是没差别的,但当指定类型时,`filter2`却需要多指定一个泛型.多此一举
...
在参数前使用`?`修饰,可以使参数变为可选参数.对于未指定默认值的可选参数,其类型永远都是声明类型本身与`undefined`的联合类型.因为在Js中未赋值的参数的值就是`undefined`.
...
与ES6一样,可以对参数设定默认值,设定默认值后,改参数变为可选参数,但不传该值或传入的值为undefined时,参数获得改默认值,并且值的类型为其本身类型,不会存在`undefined`
...
这对于定义回调很有用,使得我们可以在形如`Array.prototype.forEach`的函数中调用`Array.prototype.push`这样返回函数的值
