...
Extend.prototype = {
	...
	showBar: function () {
		...
		$(document).scroll(function () {
			if ($(this).scrollTop() > 100) oEle.fadeIn();
			else oEle.fadeOut();
		});
		this.returnTop(oEle);
	},
	...
	searchToggle: function () {
		let searchPanel = $("#local-search-result");
		let searchInput = $("#local-search-input");
		searchInput.on("focus", () => searchPanel.delay(300).slideDown());
		searchInput.on("blur", () => searchPanel.hide());
	},
};
...
window.onscroll = function autoFix() {
	let element = $(".toolbar");
	let offsetTop = element.offset().top - 55;
	let scrollTop = window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop || 0;
	console.log(`scrollTop: ${scrollTop} offsetTop: ${offsetTop}`)
	if ((scrollTop >= offsetTop) & (offsetTop >= 309)) {
		element.addClass("tool-area-fix");
	} else {
		element.removeClass("tool-area-fix");
	}
};
